#!/usr/bin/env python3
"""
Optimizer V5_pp - Scheduling con Post-Processing Repair per H9

Approccio: Risolve senza H9 nel modello CP (veloce), poi ripara le sovrapposizioni
con modifiche locali (cambio fascia o spostamento giorno).
...
- H9: No sovrapposizioni formatrice (riparato post-hoc)
...
class OptimizerV5:
    ...
    def __init__(self, data: DataLoader, verbose: bool = False, enable_soft: bool = True):
        ...
        # Soluzione riparata (post-processing)
        self.soluzione_riparata = None

    def get_value(self, key, campo):
        """Helper per leggere valori dalla soluzione riparata o dal solver."""
        if self.soluzione_riparata is not None:
            return self.soluzione_riparata[key][campo]
        else:
            if campo == 'settimana':
                return self.solver.Value(self.settimana[key])
            elif campo == 'giorno':
                return self.solver.Value(self.giorno[key])
            elif campo == 'fascia':
                return self.solver.Value(self.fascia[key])
            elif campo == 'formatrice':
                return self.solver.Value(self.formatrice[key])
            else:
                raise ValueError(f"Campo sconosciuto: {campo}")
    ...
    def build_model(self):
        ...
        for _, row in self.data.fasce_orarie_classi.iterrows():
            ...
            if pd.isna(fasce_str) or fasce_str == '':
                fasce = list(range(1, 13))
            else:
                fasce = [int(f.strip()) for f in str(fasce_str).split(',') if f.strip().isdigit()]
                if not fasce:
                    fasce = list(range(1, 13))
        ...
        for classe_id, labs in self.labs_per_classe.items():
            fasce_classe = fasce_per_classe.get(classe_id, list(range(1, 13)))
        ...
        for f_id, disp in self.disponibilita_formatrice.items():
            ...
            # Caso 2: Ha disponibilità mattina/pomeriggio per giorno
                ...
                for giorno_no in range(6):
                    for fascia_id in range(1, 13):
                        is_mattina = fascia_id in fasce_mattina
                        is_pomeriggio = fascia_id in fasce_pomeriggio
                        ...
                        if not (mattina_ok and pomeriggio_ok):
                            forbidden_tuples.append([giorno_no, fascia_id, f_id])
    ...
    def repair_h9_overlaps(self, max_iterations: int = 100):
        """
        Post-processing repair: modifica localmente la soluzione per eliminare le sovrapposizioni H9.

        Approccio:
        1. Trova tutte le sovrapposizioni (formatrice, sett, giorno con fasce sovrapponibili)
        2. Per ogni sovrapposizione, prova in ordine:
           a. Cambiare la fascia di un incontro con una non sovrapponibile (stessa classe, stesso giorno)
           b. Spostare un incontro a un altro giorno della stessa settimana (se disponibile)
        3. Verifica che la modifica non violi altri vincoli hard
        4. Ripeti fino a eliminare tutte le sovrapposizioni o raggiungere max_iterations

        Returns:
            Numero di sovrapposizioni risolte
        """
        print("\n" + "=" * 60)
        print("POST-PROCESSING REPAIR: Riparazione sovrapposizioni H9")
        print("=" * 60)

        # Prepara strutture per verifica veloce
        # Incontri secondi in accorpamenti (non modificabili direttamente)
        ...
        # Prepara dominio fasce per ogni classe
        fasce_per_classe = {}
        for _, row in self.data.fasce_orarie_classi.iterrows():
            fasce_str = row.get('fasce_disponibili', '')
            if pd.isna(fasce_str) or fasce_str == '':
                ...
                if not fasce:
                    fasce = list(range(1, 13))
            fasce_per_classe[row['classe_id']] = set(fasce)

        # Prepara mappa di date escluse per classe
        date_escluse = defaultdict(list)
        for _, row in self.data.date_escluse_classi.iterrows():
            classe_id = row['classe_id']
            parsed = DateParser.parse_date_escluse(row['date_escluse'])
        ...
        # Prepara mappa di date fissate (incontri non modificabili)
        incontri_fissati = set()
        for _, row in self.data.laboratori_classi.iterrows():
            if pd.isna(row.get('date_fissate')) or row['date_fissate'] == '':
                continue
            classe_id = row['classe_id']
            ...
            parsed = DateParser.parse_date_fissate(row['date_fissate'])
            num_inc = self.num_incontri_lab.get(lab_id, 1)
            for k in range(min(len(parsed), num_inc)):
                incontri_fissati.add((classe_id, lab_id, k))

        # Mappa corrente della soluzione
        soluzione = {}
        for key in self.tutti_incontri:
            soluzione[key] = {
                'settimana': self.solver.Value(self.settimana[key]),
                'giorno': self.solver.Value(self.giorno[key]),
                'fascia': self.solver.Value(self.fascia[key]),
                'formatrice': self.solver.Value(self.formatrice[key]),
            }

        def trova_sovrapposizioni():
            """Trova tutte le sovrapposizioni H9 nella soluzione corrente."""
            incontri_per_formatrice_giorno = defaultdict(list)
            for key in self.tutti_incontri:
                if key in incontri_secondi:
                    continue
                sol = soluzione[key]
                incontri_per_formatrice_giorno[(sol['formatrice'], sol['settimana'], sol['giorno'])].append((key, sol['fascia']))
        ...
        def fasce_non_sovrapposte_a(fascia):
            """Restituisce l'insieme di fasce che NON si sovrappongono con fascia."""
            s, e = self.fascia_orari.get(fascia, (480, 600))
            non_sovr = set()
            for fid, (s2, e2) in self.fascia_orari.items():
                if not (s < e2 and s2 < e):  # Non si sovrappongono
                    non_sovr.add(fid)
            return non_sovr

        def valida_modifica(key, nuova_sett, nuovo_giorno, nuova_fascia):
            """Verifica se una modifica è valida rispetto ai vincoli hard (tranne H9)."""
            classe_id, lab_id, k = key

            # H2: Max 1 incontro/settimana per classe
            settimane_classe = []
            for l in self.labs_per_classe[classe_id]:
                for kk in range(self.num_incontri_lab.get(l, 1)):
                    if (classe_id, l, kk) == key:
                        settimane_classe.append(nuova_sett)
                    else:
                        settimane_classe.append(soluzione[(classe_id, l, kk)]['settimana'])
            if len(settimane_classe) != len(set(settimane_classe)):
                return False

            # H12: Date escluse
            for sett_esc, giorno_esc, fasce_escluse in date_escluse[classe_id]:
                if sett_esc is None:
                    continue
                if nuova_sett == sett_esc and nuovo_giorno == giorno_esc:
                    if fasce_escluse is None or nuova_fascia in fasce_escluse:
                        return False

            # GSSI: Settimane bloccate
            if nuova_sett in self.settimane_gssi.get(classe_id, set()):
                return False

            # H13: Date fissate (non modificabili)
            if key in incontri_fissati:
                return False

            # Vincoli calendario (settimana 0 solo da giovedì, settimana 9 fino a mercoledì, etc.)
            if nuova_sett == 0 and nuovo_giorno < 3:
                return False
            if nuova_sett == self.NUM_SETTIMANE - 1 and nuovo_giorno > 3:
                return False
            if nuova_sett == 9 and nuovo_giorno > 2:
                return False
            ...
            if nuovo_giorno == 5:
                if scuola_id not in self.SCUOLE_SABATO:
                    return False
                # In questo caso dovremmo anche verificare che la formatrice sia 4
                # Ma per semplicità, assumiamo che la soluzione iniziale rispetti H7

            # Fasce valide per la classe
            if nuova_fascia not in fasce_per_classe.get(classe_id, set(range(1, 13))):
                return False

            return True

        def applica_modifica(key, nuova_sett, nuovo_giorno, nuova_fascia):
            """Applica la modifica alla soluzione corrente (e a eventuali accorpamenti)."""
            soluzione[key]['settimana'] = nuova_sett
            soluzione[key]['giorno'] = nuovo_giorno
            soluzione[key]['fascia'] = nuova_fascia

            # Se è un incontro accorpato (primo in coppia), aggiorna anche il secondo
            if key in accorpamenti_attivi:
                c1, lab_id, k = key
                c2 = accorpamenti_attivi[key]
                key2 = (c2, lab_id, k)
                soluzione[key2]['settimana'] = nuova_sett
                soluzione[key2]['giorno'] = nuovo_giorno
                soluzione[key2]['fascia'] = nuova_fascia

        # Main repair loop
        n_risolte = 0
        for iteration in range(max_iterations):
            overlaps = trova_sovrapposizioni()

            if not overlaps:
                print(f"\n  Iterazione {iteration + 1}: NESSUNA sovrapposizione! Riparazione completata.")
                break

            print(f"\n  Iterazione {iteration + 1}: {len(overlaps)} sovrapposizioni trovate")

            risolte_questa_iter = 0

            for key1, key2, form_id, sett, giorno, fascia1, fascia2 in overlaps:
                # Prova a risolvere modificando key1 o key2 (scegli quello non fissato)
                for key_da_modificare in [key1, key2]:
                    if key_da_modificare in incontri_secondi:
                        continue  # Non modificare incontri secondi in accorpamenti
                    if key_da_modificare in incontri_fissati:
                        continue  # Non modificare incontri fissati

                    classe_id, lab_id, k = key_da_modificare
                    sol_curr = soluzione[key_da_modificare]
                    fascia_curr = sol_curr['fascia']
                    fascia_altra = fascia1 if key_da_modificare == key2 else fascia2

                    # Strategia 1: Cambia fascia (stesso giorno)
                    fasce_valide = fasce_per_classe.get(classe_id, set(range(1, 13)))
                    fasce_non_sovr = fasce_non_sovrapposte_a(fascia_altra)
                    candidate_fasce = fasce_valide & fasce_non_sovr

                    for nuova_fascia in candidate_fasce:
                        if valida_modifica(key_da_modificare, sol_curr['settimana'], sol_curr['giorno'], nuova_fascia):
                            applica_modifica(key_da_modificare, sol_curr['settimana'], sol_curr['giorno'], nuova_fascia)
                            risolte_questa_iter += 1
                            break  # Risolto, passa alla prossima sovrapposizione
                    else:
                        # Strategia 2: Sposta a un altro giorno della stessa settimana
                        scuola_id = self.scuola_per_classe[classe_id]
                        max_giorno = 5 if scuola_id in self.SCUOLE_SABATO else 4
                        for nuovo_giorno in range(max_giorno + 1):
                            if nuovo_giorno == sol_curr['giorno']:
                                continue
                            if valida_modifica(key_da_modificare, sol_curr['settimana'], nuovo_giorno, sol_curr['fascia']):
                                applica_modifica(key_da_modificare, sol_curr['settimana'], nuovo_giorno, sol_curr['fascia'])
                                risolte_questa_iter += 1
                                break  # Risolto

                    if risolte_questa_iter > 0:
                        break  # Risolto, passa alla prossima sovrapposizione

            n_risolte += risolte_questa_iter
            print(f"    Risolte: {risolte_questa_iter}")

            if risolte_questa_iter == 0:
                print("\n  ATTENZIONE: Nessuna sovrapposizione risolta in questa iterazione. Interrompo.")
                break

        # Aggiorna i valori nel solver (per export e verify)
        # NOTA: Non possiamo modificare i valori delle variabili del solver, quindi
        # dobbiamo memorizzare la soluzione modificata per l'export
        self.soluzione_riparata = soluzione

        overlaps_finali = trova_sovrapposizioni()
        print("\n" + "=" * 60)
        print(f"RIPARAZIONE COMPLETATA: {len(overlaps_finali)} sovrapposizioni residue")
        print("=" * 60)

        return len(overlaps_finali)
    ...
    def verify_constraints(self):
        ...
        for classe_id, labs in self.labs_per_classe.items():
            ...
            for lab_id in labs:
                for k in range(self.num_incontri_lab.get(lab_id, 1)):
                    key = (classe_id, lab_id, k)
                    sett = self.get_value(key, 'settimana')
        ...
        for classe_id, labs in self.labs_per_classe.items():
            if self.LAB_PRESENTAZIONE_MANUALI not in labs:
                continue
            sett_pm = self.get_value((classe_id, self.LAB_PRESENTAZIONE_MANUALI, 0), 'settimana')
            for lab_id in labs:
                if lab_id == self.LAB_PRESENTAZIONE_MANUALI:
                    continue
                num_inc = self.num_incontri_lab.get(lab_id, 1)
                sett_ultimo = self.get_value((classe_id, lab_id, num_inc - 1), 'settimana')
        ...
        for classe_id, labs in self.labs_per_classe.items():
            ...
            for i, lab_a in enumerate(labs_senza_8):
                for lab_b in labs_senza_8[i+1:]:
                    num_a = self.num_incontri_lab.get(lab_a, 1)
                    num_b = self.num_incontri_lab.get(lab_b, 1)
                    ultimo_a = self.get_value((classe_id, lab_a, num_a - 1), "settimana")
                    primo_b = self.get_value((classe_id, lab_b, 0), "settimana")
                    ultimo_b = self.get_value((classe_id, lab_b, num_b - 1), "settimana")
                    primo_a = self.get_value((classe_id, lab_a, 0), "settimana")
        ...
        for key in self.tutti_incontri:
            classe_id = key[0]
            giorno = self.get_value(key, "giorno")
            if giorno == 5:  # Sabato
                scuola_id = self.scuola_per_classe[classe_id]
                if scuola_id not in self.SCUOLE_SABATO:
                    h5_violations += 1
                    errors.append(f"H5: Scuola {scuola_id} lavora sabato (non permesso)")
                form_id = self.get_value(key, "formatrice")
        ...
        for key in self.tutti_incontri:
            ...
            sett = self.get_value(key, "settimana")
            giorno = self.get_value(key, "giorno")
            fascia = self.get_value(key, "fascia")
        ...
        for (c1, c2, lab_id) in accorpamenti_attivi:
            for k in range(self.num_incontri_lab.get(lab_id, 1)):
                ...
                s1, s2 = self.get_value(key1, "settimana"), self.get_value(key2, "settimana")
                g1, g2 = self.get_value(key1, "giorno"), self.get_value(key2, "giorno")
                f1, f2 = self.get_value(key1, "fascia"), self.get_value(key2, "fascia")
                fo1, fo2 = self.get_value(key1, "formatrice"), self.get_value(key2, "formatrice")
        ...
        for _, form in self.data.formatrici.iterrows():
            ...
            ore_effettive = 0
            for key in self.tutti_incontri:
                if self.get_value(key, "formatrice") == f_id:
    ...
    def verify_soft_constraints(self):
        ...
        for classe_id, labs in self.labs_per_classe.items():
            ...
            for lab_id in labs:
                ...
                for k in range(num_inc):
                    key = (classe_id, lab_id, k)
                    f_id = self.get_value(key, "formatrice")
    ...
    def export_results(self, output_path: str):
        ...
        # Mappa fascia_id -> nome (es. 1 -> "8-10")
        fascia_nome = {}
        for _, row in self.data.fasce_orarie_scuole.iterrows():
            fid = int(row['fascia_id'])
            if fid not in fascia_nome:
                fascia_nome[fid] = row['nome']
        ...
        for key in self.tutti_incontri:
            ...
            risultati.append({
                ...
                'fascia': fascia_nome.get(fascia, str(fascia)),
...
def main():
    parser = argparse.ArgumentParser(description='Optimizer V5_pp - Con post-processing repair per H9')
    ...
    parser.add_argument('--output', '-o', type=str, default='data/output/calendario_V5_pp.csv', help='File output')
    parser.add_argument('--no-soft', action='store_true', help='Disabilita vincoli soft (solo hard constraints)')
    parser.add_argument('--max-repair-iterations', type=int, default=100, help='Max iterazioni post-processing repair H9')
    args = parser.parse_args()

    enable_soft = not args.no_soft
    mode = "Con Vincoli Soft + Post-Processing" if enable_soft else "SOLO Hard Constraints + Post-Processing"

    print("=" * 60)
    print(f"  OPTIMIZER V5_pp - {mode}")
    print("=" * 60)
    ...
    # Risolvi senza H9 (veloce)
    if not optimizer.solve(time_limit_seconds=args.timeout):
        print("\nNessuna soluzione trovata.")
        sys.exit(1)

    # Post-processing repair: ripara sovrapposizioni H9 con modifiche locali
    optimizer.repair_h9_overlaps(max_iterations=args.max_repair_iterations)
